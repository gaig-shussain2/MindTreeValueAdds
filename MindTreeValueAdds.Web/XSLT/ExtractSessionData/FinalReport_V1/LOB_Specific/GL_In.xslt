...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
  ...
  <xsl:template name="GL_Base" match="/" xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:html="http://www.w3.org/TR/REC-html40">
    <Worksheet ss:Name="Coverages Detailed Report">
      ...
      <Table ss:ExpandedColumnCount="16" x:FullColumns="1" x:FullRows="1" ss:DefaultRowHeight="14.4375">
        ...
        <Column ss:StyleID="s86" ss:Width="89.25"/>
        <Column ss:StyleID="s86" ss:Width="165.75"/>
        ...
        <Row ss:AutoFitHeight="0" ss:Height="15" ss:StyleID="s87">
          ...
          <Cell ss:StyleID="s88">
            <Data ss:Type="String">Premium Basis</Data>
            <NamedCell ss:Name="_FilterDatabase"/>
          </Cell>
          <Cell ss:StyleID="s89">
        ...
        <xsl:variable name="LinePath">//session/data/policy/line[Type='GeneralLiability']</xsl:variable>
        <xsl:variable name="DefaultPathLine">//session/data/policy/line[Type='GeneralLiability']/coverage</xsl:variable>
        <xsl:variable name="DefaultPathRisk">//session/data/policy/line[Type='GeneralLiability']/risk/coverage</xsl:variable>

        <!--Line: Start-->
        <xsl:for-each select="//session/data/policy/line[Type='GeneralLiability']/coverage[(Indicator='1' or @deleted='1') and Type!='AdditionalInterests' and Type!='LineFinanceCharge']">
          <xsl:call-template name="BuildCoverageRow_GL">
            ...
            <xsl:with-param name="Classification" select="'N/A'" />
            <xsl:with-param name="Type" select="Type" />
            <xsl:with-param name="PremiumBasis" select="'N/A'" />
            ...
            <xsl:with-param name="Xpath" select="concat($DefaultPathLine, '[Type=', $apos, Type, $apos, ']')" />
          </xsl:call-template>
        </xsl:for-each>

        <!--Risk: Start-->
        <xsl:for-each select="//session/data/policy/line[Type='GeneralLiability']/risk/coverage[(Indicator='1' or @deleted='1')]">
          ...
          <xsl:call-template name="BuildCoverageRow_GL">
            ...
            <xsl:with-param name="ClassCode" select="statCode[Type='Class']/sValue" />
            <xsl:with-param name="Subline" select="statCode[Type='Subline']/sValue" />
            <xsl:with-param name="Xpath" select="concat($DefaultPathRisk, '[Type=', $apos, Type, $apos, ']')" />
          </xsl:call-template>
        </xsl:for-each>

        <!--Additional Interests: Start-->
        <xsl:for-each select="//session/data/policy/line[Type='GeneralLiability']/coverage[Type='AdditionalInterests' and (Indicator='1' or @deleted='1')]">
          <xsl:call-template name="BuildCoverageRow_GL">
            <xsl:with-param name="CoverageName" select="exposure['DetailDescription']/Description" />
            <xsl:with-param name="Indicator" select="Indicator" />
            <xsl:with-param name="Deleted" select="@deleted" />
            <xsl:with-param name="Location" select="'N/A'" />
            <xsl:with-param name="Classification" select="'N/A'" />
        ...
        <!--Location Terrorism: Start-->
        <xsl:for-each select="//session/data/account/location/coverage[Type='LocationTerrorism' and (Indicator='1' or @deleted='1')]">
          <xsl:call-template name="BuildCoverageRow_GL">
            <xsl:with-param name="CoverageName" select="'Location Terrorism'" />
            <xsl:with-param name="Indicator" select="Indicator" />
            <xsl:with-param name="Deleted" select="@deleted" />
            <xsl:with-param name="Location" select="../Description" />
            <xsl:with-param name="Classification" select="'N/A'" />
            <xsl:with-param name="Type" select="Type" />
            <xsl:with-param name="PremiumBasis" select="'N/A'" />
            <xsl:with-param name="Level" select="'Location'" />
            <xsl:with-param name="Premium" select="Premium" />
            <xsl:with-param name="Written" select="written" />
            <xsl:with-param name="Change" select="change" />
            <xsl:with-param name="Limit" select="limit[Type='Standard']/iValue" />
            ...
            <xsl:with-param name="ClassCode" select="statCode[Type='Class']/sValue" />
            <xsl:with-param name="Subline" select="statCode[Type='Subline']/sValue" />
            <xsl:with-param name="Xpath" select="concat('//session/data/account/location/coverage', '[Type=', $apos, Type, $apos, ']')" />
          </xsl:call-template>
        </xsl:for-each>

        <!--Misc. All Other & Manuscript Coverages: Start-->
        <xsl:for-each select="//session/data/policy/line[Type='GeneralLiability']/ManualCoverage/coverage[Type='MiscAllOther' or Type='ManuscriptCoverage' and (Indicator='1' or @deleted='1')]">
          <xsl:variable name="_level" >
            <xsl:choose>
              <xsl:when test="LocationID and LocationID != ''">
          ...
          <xsl:variable name="LocationID" select="LocationID" />
          <xsl:call-template name="BuildCoverageRow_GL">
            <xsl:with-param name="CoverageName" select="CoverageTitle" />
            <xsl:with-param name="Indicator" select="Indicator" />
            <xsl:with-param name="Deleted" select="@deleted" />
            <xsl:with-param name="Location" select="//session/data/account/location[@id=$LocationID]/Description" />
            <xsl:with-param name="Classification" select="'N/A'" />
            <xsl:with-param name="Type" select="Type" />
            <xsl:with-param name="PremiumBasis" select="'N/A'" />
            <xsl:with-param name="Level" select="$_level" />
            <xsl:with-param name="Premium" select="Premium" />
            <xsl:with-param name="Written" select="written" />
            <xsl:with-param name="Change" select="change" />
            <xsl:with-param name="Limit" select="limit[Type='Standard']/iValue" />
            ...
            <xsl:with-param name="ClassCode" select="statCode[Type='Class']/sValue" />
            <xsl:with-param name="Subline" select="statCode[Type='Subline']/sValue" />
            <xsl:with-param name="Xpath" select="concat($LinePath, '/ManualCoverage/coverage', '[Type=', $apos, Type, $apos, ']')" />
          </xsl:call-template>
        </xsl:for-each>

        <!--Finance Charge: Start-->
        <xsl:for-each select="//session/data/policy/line[Type='GeneralLiability']/coverage[Type='LineFinanceCharge' and (Indicator='1' or @deleted='1')]">
          <xsl:call-template name="BuildCoverageRow_GL">
  ...
  <xsl:template name="BuildCoverageRow_GL"  xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
